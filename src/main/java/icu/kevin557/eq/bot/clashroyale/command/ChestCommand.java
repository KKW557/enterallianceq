...
import icu.kevin557.eq.bot.clashroyale.image.ChestImage;
...
public class ChestCommand extends Command {
    ...
    public ChestCommand(EnterallianceQ bot) {
        super(bot, "clashroyale.chest");
    }
            ...
            Executor.run(() -> {
                try {
                    ...
                    if (body.get("reason") == null) {
                        ChatUtils.replay(event, new ChestImage(this.bot, body.getJSONArray("items")).get(), tag);
                    } else {
